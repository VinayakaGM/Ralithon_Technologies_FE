...
import { Calendar, Clock, CheckCircle, AlertCircle, ChevronDown, ChevronUp, Trophy, Target, TrendingUp, Award } from "lucide-react";
...
export default function AssessmentsTab() {
  ...
  const getProgressColor = (percentage: number) => {
    if (percentage >= 90) return "bg-gradient-to-r from-emerald-500 to-teal-500";
 ...
 const getBadgeClass = (type: string) => {
  ...
  const groupedAssessments: GroupedAssessment[] = assessments
    .filter(assessment => assessment.assessmentSubmit && assessment.assessmentSubmit.length > 0)
    .map(assessment => {
      ...
      const bestScore = Math.max(...submissions.map(sub => sub.assessmentScored));
      const bestPercentage = Math.round((bestScore / firstSubmission.totalScored) * 100);
      const averageScore = Math.round(submissions.reduce((sum, sub) => sum + sub.assessmentScored, 0) / submissions.length);

      // Calculate improvement (latest vs first attempt)
      const improvement = submissions.length > 1 ?
        sortedSubmissions[0].assessmentScored - sortedSubmissions[sortedSubmissions.length - 1].assessmentScored : 0;
                    ...
                    <div className="bg-white/70 backdrop-blur-sm border border-white/20 rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-2xl font-bold text-gray-900">{groupedAssessments.length}</p>
                        </div>
                      </div>
                    </div>
                    <div className="bg-white/70 backdrop-blur-sm border border-white/20 rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-2xl font-bold text-gray-900">
                            {Math.max(...groupedAssessments.map(a => a.bestPercentage))}%
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="bg-white/70 backdrop-blur-sm border border-white/20 rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-2xl font-bold text-gray-900">
                            {groupedAssessments.reduce((sum, a) => sum + a.totalAttempts, 0)}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  {groupedAssessments.length === 0 ? (
                    <div className="text-center py-16">
                      ...
                      <p className="mt-2 text-gray-500 max-w-md mx-auto">
                        Start taking assessments to see your progress and performance metrics here.
                      </p>
                    </div>
                  ) : (
                    groupedAssessments.map((assessment) => (
                      <Card key={assessment.assessmentId} className="bg-white/80 backdrop-blur-sm border border-white/20 shadow-lg hover:shadow-xl transition-all duration-500 hover:-translate-y-1 overflow-hidden">
                        ...
                        <CardHeader className="pb-4">
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex items-center gap-3">
                              <Badge variant="secondary" className="bg-blue-50 text-blue-700 border border-blue-200 px-3 py-1 rounded-full">
                                {assessment.totalAttempts} attempt{assessment.totalAttempts > 1 ? 's' : ''}
                              </Badge>
                        ...
                        <CardContent className="pt-0">
                          <div className="space-y-6">
                            ...
                            <div className="bg-gradient-to-br from-gray-50 to-white rounded-xl p-6 border border-gray-100">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="text-center">
                                  ...
                                  <p className={`text-3xl font-bold ${getScoreColor(assessment.bestPercentage)}`}>
                                    {assessment.bestPercentage}%
                                  </p>
                                  <p className="text-sm text-gray-500">
                                    {assessment.bestScore}/{assessment.submissions[0].totalScored}
                                  </p>
                                </div>
                                <div className="text-center">
                                  <p className="text-sm font-medium text-gray-600 mb-2">Average Score</p>
                                  <p className="text-3xl font-bold text-blue-600">
                                    {Math.round((assessment.averageScore / assessment.submissions[0].totalScored) * 100)}%
                                  </p>
                                  <p className="text-sm text-gray-500">
                                    {assessment.averageScore}/{assessment.submissions[0].totalScored}
                                  </p>
                                </div>
                                <div className="text-center">
                                  <p className="text-sm font-medium text-gray-600 mb-2">Improvement</p>
                                  <p className={`text-3xl font-bold ${assessment.improvement >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                              ...
                              <div className="mt-6">
                                <div className="flex items-center justify-between text-sm mb-2">
                                  ...
                                  <span className={`font-bold ${getScoreColor(assessment.bestPercentage)}`}>
                                    {assessment.bestPercentage}%
                                  </span>
                                </div>
                                <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                                  <div
                                    className={`h-full ${getProgressColor(assessment.bestPercentage)} transition-all duration-1000 ease-out rounded-full shadow-lg`}
                                    style={{ width: `${assessment.bestPercentage}%` }}
                                  ></div>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <Button
                                variant="outline"
                                onClick={() => toggleExpanded(assessment.assessmentId)}
                                      ...
                                      <div className="flex items-center justify-between mb-3">
                                        <div className="flex items-center gap-3">
                                          <div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full flex items-center justify-center text-white text-sm font-bold">
                                            {assessment.submissions.length - index}
                                          </div>
                                          <span className="font-medium text-gray-900">
                                            Attempt {assessment.submissions.length - index}
                                          </span>
                                        </div>
                                        <Badge variant="outline" className="text-sm font-semibold">
                                          {Math.round((submission.assessmentScored / submission.totalScored) * 100)}%
                                        </Badge>
                                      </div>
                                      ...
                                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-3">
                                        <div className="bg-white rounded-lg p-3 text-center border border-gray-100">
                                          ...
                                          <p className="font-bold text-gray-900">
                                            {submission.assessmentScored}/{submission.totalScored}
                                          </p>
                                        </div>
                                        <div className="bg-white rounded-lg p-3 text-center border border-gray-100">
                                          <p className="text-gray-600">Attempted</p>
                                          <p className="font-bold text-blue-600">
                                            {submission.attemptQuestion}/{submission.totalQuestion}
                                          </p>
                                        </div>
                                        <div className="bg-white rounded-lg p-3 text-center border border-gray-100">
                                          <p className="text-gray-600">Correct</p>
                                          <p className="font-bold text-emerald-600">{submission.correctAnswer}</p>
                                        </div>
                                        <div className="bg-white rounded-lg p-3 text-center border border-gray-100">
                                          <p className="text-gray-600">Incorrect</p>
                                          <p className="font-bold text-red-600">{submission.incorrectAnswer}</p>
                                        </div>
                                      </div>
